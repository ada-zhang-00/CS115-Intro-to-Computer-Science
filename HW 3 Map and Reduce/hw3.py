...
def sumnums(vectin):
    return reduce(lambda x,y: x+y, vectin)
...
def taylorApproxE(lastIter):
    return sumnums(list(map(lambda x: 1/factorial(x), range(lastIter+1))))
...
def vectorNorm(vect1):
    return sqrt(sumnums(list(map(lambda x: x**2, vect1))))
...
# Add a suitable docstring to the function and delete this comment block
def arithMean(vect1):
    return  sumnums(vect1)/len(vect1)
